...
internal class ValueClassAwareCaller<out M : Member?>(
    ...
    private class UnboxData(val argumentRange: IntRange, val unboxParameters: Array<List<Method>?>, val box: Method?)

    private val data: UnboxData = run {
        ...
        if (descriptor.isGetterOfUnderlyingPropertyOfValueClass()) {
            ...
            return@run UnboxData(IntRange.EMPTY, emptyArray(), box)
        }
        ...
        val flattenedShift = if (caller is CallerImpl.Method.BoundStaticMultiFieldValueClass) -caller.receiversCount else shift
        ...
        val extraArgumentsTail =
            (if (isDefault) ((kotlinParameterTypes.sumOf(::typeSize) + Integer.SIZE - 1) / Integer.SIZE) + 1 else 0) +
        ...
        UnboxData(argumentRange, unbox, box)
    }
    ...
    override fun call(args: Array<*>): Any? {
        ...
        val unboxedArguments = if (range.isEmpty()) {
            ...
            for (index in range) {
                ...
                val arg = args[index]
                // Note that arg may be null in case we're calling a $default method, and it's an optional parameter of a multi-field value class type
                if (methods != null) {
...
internal fun getMfvcUnboxMethods(type: SimpleType): List<Method>? {
    ...
    val unboxMethods = getUnboxMethodNameSuffixes(type.asSimpleType()) ?: return null
    val methods = (type.constructor.declarationDescriptor as ClassDescriptor).toJavaClass()!!.declaredMethods
    return unboxMethods.map { name -> methods.single { it.modifiers and Modifier.STATIC == 0 && it.name == "unbox-impl-$name" } }
}
...
private fun makeKotlinParameterTypes(descriptor: CallableMemberDescriptor, isSpecificClass: ClassDescriptor.() -> Boolean) =
    ArrayList<KotlinType>().also { kotlinParameterTypes ->
...
internal fun <M : Member?> Caller<M>.createValueClassAwareCallerIfNeeded(
    ...
    val needsValueClassAwareCaller: Boolean =
        descriptor.isGetterOfUnderlyingPropertyOfValueClass() ||
                descriptor.extensionReceiverParameter.let { it != null && it.type.isValueClassType() } ||
                descriptor.dispatchReceiverParameter.let { it != null && it.type.isValueClassType() } ||
                ...
                this !is BoundCaller && descriptor.hasValueClassReceiver()
