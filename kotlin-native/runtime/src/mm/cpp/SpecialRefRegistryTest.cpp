...
public:
    ...
    std::vector<ObjHeader*> all(Invalidated&&... invalidated) noexcept {
        ...
        for (auto& obj : mm::SpecialRefRegistry::instance().lockForIter()) {
            if (invalidatedSet.find(obj.load(std::memory_order_relaxed)) != invalidatedSet.end()) {
                obj.store(nullptr, std::memory_order_release);
            }
            result.push_back(obj.load(std::memory_order_relaxed));
        }
        return result;
    }
