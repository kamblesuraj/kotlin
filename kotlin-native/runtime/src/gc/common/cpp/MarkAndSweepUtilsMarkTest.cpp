...
void InstallWeakCounter(test_support::Any& object, test_support::WeakReferenceCounter& counter) noexcept {
    auto& extraObjectData = mm::ExtraObjectData::GetOrInstall(object.header());
    counter->referred = object.header();
    auto* setCounter = extraObjectData.GetOrSetWeakReferenceCounter(object.header(), counter.header());
    EXPECT_EQ(setCounter, counter.header());
...
TEST_F(MarkAndSweepUtilsMarkTest, MarkSingleObjectWithWeakCounter) {
    test_support::WeakReferenceCounter weakCounter;
    Object object;
    InstallWeakCounter(object, weakCounter);
...
TEST_F(MarkAndSweepUtilsMarkTest, MarkSingleObjectArrayWithWeakCounter) {
    test_support::WeakReferenceCounter weakCounter;
    ObjectArray array;
    InstallWeakCounter(array, weakCounter);
