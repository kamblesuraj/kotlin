...
object FirExpectActualResolver {
        ...
        if (
            expectSupertypes.map(substitutor::substituteOrSelf).any { expectSupertype ->
                actualSupertypes.none { actualSupertype ->
                    areCompatibleExpectActualTypes(expectSupertype, actualSupertype, expectSession, actualSession)
                }
            }
        ) {
    ...
    private fun areCompatibleTypeLists(
        ...
        for (i in expectedTypes.indices) {
            if (!areCompatibleExpectActualTypes(expectedTypes[i], actualTypes[i], expectSession, actualSession)) {
                return false
            }
        }
        return true
