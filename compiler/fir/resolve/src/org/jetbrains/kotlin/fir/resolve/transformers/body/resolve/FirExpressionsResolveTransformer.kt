    ...
    override fun transformIncrementDecrementExpression(
        ...
        val generator = object {
            fun generateTemporaryVariable(name: Name, initializer: FirExpression): FirProperty = generateTemporaryVariable(
                ...
                withResolvedStatus = true,
            )

            fun buildOperatorCall(receiver: FirExpression): FirFunctionCall = buildFunctionCall {
                source = incrementDecrementExpression.operationSource
                explicitReceiver = receiver
                calleeReference = buildSimpleNamedReference {
                    source = incrementDecrementExpression.operationSource?.fakeElement(KtFakeSourceElementKind.ReferenceInAtomicQualifiedAccess)
                    name = incrementDecrementExpression.operationName
                }
                origin = FirFunctionCallOrigin.Operator
            }.transformSingle(transformer, ResolutionMode.ContextIndependent)

            fun buildVariableAssignment(rValue: FirExpression): FirVariableAssignment = buildVariableAssignment {
                source = desugaredSource
        ...
        val block = buildBlock {
            ...
            (expression as? FirQualifiedAccessExpression)?.explicitReceiver
                ...
                ?.let { receiver ->
                    // val <receiver> = x
                    statements += generator.generateTemporaryVariable(SpecialNames.RECEIVER, receiver).also { property ->
                        // Change the expression from x.a to <receiver>.a
                        val newReceiverAccess = property.toQualifiedAccess()
            ...
            if (incrementDecrementExpression.isPrefix) {
                ...
                val operatorCall = generator.buildOperatorCall(expression)
                ...
                if (targetProperty?.isLocal == true && targetProperty.delegate == null) {
                    // a = a.inc()
                    statements += generator.buildVariableAssignment(operatorCall)
                    // ^a
                    ...
                    val unaryResultVariable = generator.generateTemporaryVariable(SpecialNames.UNARY_RESULT, operatorCall)

                    // val <unary-result> = a.inc()
                    statements += unaryResultVariable
                    // a = <unary-result>
                    statements += generator.buildVariableAssignment(unaryResultVariable.toQualifiedAccess())
                    // ^<unary-result>
                    statements += unaryResultVariable.toQualifiedAccess()
                }
            } else {
                val unaryVariable = generator.generateTemporaryVariable(SpecialNames.UNARY, expression)

                // val <unary> = a
                statements += unaryVariable
                // a = <unary>.inc()
                statements += generator.buildVariableAssignment(generator.buildOperatorCall(unaryVariable.toQualifiedAccess()))
                // ^<unary>
